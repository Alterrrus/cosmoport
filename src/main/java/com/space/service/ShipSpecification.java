...
public class ShipSpecification {
    ...
    public static Specification<Ship> shipsByRating(Double min, Double max) {
        return new Specification<Ship>() {
            @Override
            public Predicate toPredicate(Root<Ship> r, CriteriaQuery<?> q, CriteriaBuilder cb) {
                Predicate result;
                if (min == null && max == null) {
                    result = null;
                } else if (min == null) {
                    result = cb.lessThanOrEqualTo(r.get("rating"), max);
                } else if (max == null) {
                    result = cb.greaterThanOrEqualTo(r.get("rating"), min);
                } else {
                    result = cb.between(r.get("rating"), min, max);
                }
                return result;
            }
        };
